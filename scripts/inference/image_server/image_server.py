...
class OpenCVCamera():
    ...
    def _can_read_frame(self):
        ...
        return success
    # def _can_read_frame(self, retries=30, delay=0.1):
    ...
    def get_frame(self):
        ...
        # if len(color_image.shape) == 2 or color_image.shape[2] == 1:
        ...
        # 检测是否为 YUYV (有时返回2通道)
            ...
            try:
                ...
                logger_mp.warning(f"[Image Server] Failed YUYV to BGR conversion: {e}")

        # 检测是否为拼接双目（宽度是高度的两倍）
        h, w = frame.shape[:2]
        if abs(w / h - 2.0) < 0.2:  # 近似 2:1 比例认为是拼接双目
            left = frame[:, :w // 2]
            right = frame[:, w // 2:]
            # 拼接时两目都转为 BGR
            left = cv2.cvtColor(left, cv2.COLOR_BGR2BGR)
            right = cv2.cvtColor(right, cv2.COLOR_BGR2BGR)
            frame = np.hstack([left, right])  # 保持输出一致性
        return frame
        ...
        try:
            while True:
                ...
                if self.wrist_cameras:
                    ...
                    for cam in self.wrist_cameras:
                        if self.wrist_camera_type == 'opencv':
                            ...
                            if color_image is None:
                                ...
                                break
                            # if cam.id == 4: # 👇 对 ID 为 4 的相机进行上下+左右翻转（180°）
                            #     color_image = cv2.flip(color_image, -1)  # -1 = 上下+左右翻转                            
                        elif self.wrist_camera_type == 'realsense':
                            color_image, depth_iamge = cam.get_frame()
                            if color_image is None:
                                logger_mp.error("[Image Server] Wrist camera frame read is error.")
                    ...
                    # print("head_color shape:", head_color.shape)
                    # print("wrist_color shape:", wrist_color.shape)
...
if __name__ == "__main__":
    config = {
        ...
        # 'head_camera_id_numbers': ["338622071118"],  # Realsense:38622071118
        
        'wrist_camera_type': 'opencv',
        'wrist_camera_image_shape': [480, 640],  # Wrist camera resolution
        'wrist_camera_id_numbers': [4, 2]
        } # 230322270429  230422271178
